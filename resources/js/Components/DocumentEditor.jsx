...
function DocumentEditor() {
    ...
    const addImageAndDateAtCursor = async () => {
        if (editorObj.current) {
            const editor = editorObj.current.documentEditor;

            try {
                const file = await getFile('fad_chief.png'); // Fetch the image
                if (file) {
                    const reader = new FileReader();
                    reader.onload = () => {
                        editor.editor.insertImage(reader.result, 100, 50); // Insert Image at Cursor
                        editor.editor.insertText('\n' + new Date().toLocaleDateString()); // Insert Date Below
                    };
    ...
    const insertImageWithWrap = () => {
        if (editorObj) {
            const imageUrl = '/storage/signatures/qmr.png';
            editorObj.documentEditor.editor.insertImage(imageUrl,150, 80);
            editorObj.documentEditor.editor.insertText('\n');
            editorObj.documentEditor.editor.insertText('hahahah'); 
            setTimeout(() => {
                const selection = editorObj.documentEditor.selection;
                if (selection) {
                    selection.characterFormat.textWrappingStyle = 'BehindText';
                }
            }, 200);
        }
    };
    ...
    return (
        <div className='max-w-full mx-auto'>
            ...
            <DocumentEditorContainerComponent height={'90vh'}   serviceUrl="https://localhost:7087/api/documenteditor/"
