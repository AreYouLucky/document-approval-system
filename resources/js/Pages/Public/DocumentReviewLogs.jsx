        ...
        <>
            <Modal show={show} onClose={onClose} maxWidth="4xl">
                <div className="md:px-15 ">
                    <div className=" md:px-5 py-6 bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-50 rounded-xl">
                        <div className="">
                            <h2 className="text-xl w-full px-5 pt-5 text-center roboto-bold">DOCUMENT TIMELINE</h2>
                        </div>
                        <div className="w-full md:px-5 mt-2 nunito-sm">
                            <div className="w-full max-h-[80vh] px-5 pb-5 overflow-y-auto rounded-lg">
                                <ol className="relative border-s border-gray-200 dark:border-gray-700">
                                    {processing === false ?
                                        <>
                                            {document.map((revision, index) => (
                                                <li className="mt-2 mb-4 ms-6  md:p-5 rounded-lg  dark:bg-gray-800 border" key={index}>
                                                    ...
                                                    <h3 className="flex items-center mb-1 text-sm md:text-lg font-semibold text-gray-900 dark:text-white ml-2">{revision.title} v{revision.version_no}
                                                    ...
                                                    <time className="block mb-2 text-sm font-normal leading-none text-gray-800 dark:text-gray-400 ml-2">Uploaded on {convertDate(revision.date_prepared)}
                                                    </time>
                                                    ...
                                                    {revision.audit_logs && revision.audit_logs.length > 0 ? (
                                                        revision.audit_logs.map((log) => {
                                                            ...
                                                            return (
                                                                <div
                                                                    key={log.id}
                                                                    className={`ml-2 border my-3 nunito-regular px-3 py-4 relative w-full rounded-md dark:bg-gray-700 text-sm font-normal text-gray-900 dark:text-gray-100  ${matchedStatus ? matchedStatus.color : ""
                                                                        }`}
                                                                >
                                                                    <p className="leading-loose"><strong>Review Stage:</strong> {matchedStatus ? matchedStatus.title : "Unknown Stage"}</p>
                                                                    <p><strong>Status:</strong> {matchedStatus ? matchedStatus.value : "Unknown Status"}</p>
                                                                    <p><strong>Remarks:</strong> {log.remarks}</p>
                                                                    <span className="absolute right-3 md:top-3 top-1  text-gray-600 text-xs dark:text-gray-200"> {log.audit_date}</span>
{/* 
