...
export default function DcDocumentTabs({ children }) {
    const tabs = [
    ...
    const getProcessesCount = () => {
        axios.get('/dc/get-processes-count').then((res) => {
            setData(res.data);
        })
    };
    const division = [
            ...
            <div className='w-full  px-4 py-4'>
                ...
                <div className="w-full bg-white dark:bg-gray-800 rounded-lg py-5 px-10 shadow-lg  dark:border-gray-950 ">
                    <div className="w-full flex flex-row border-b">
                        <Link className={` ${base} ${url === "/dc/initial-review-list" ? on : off} `} href='/dc/initial-review-list'>
                            <div className="relative flex items-center gap-2">
                                <GrDocumentTime />
                                <span className='nunito-regular'>For Initial Review</span>
                                {Number(data.initial_review) > 0 && <Badge className="bg-blue-200 text-slate-800 rounded-xl text-xs">{data.initial_review}</Badge>}
                            </div>
                        </Link>
                        <Link className={` ${base} ${url === "/dc/final-review-list" ? on : off} `} href='/dc/final-review-list'>
                            <div className="relative flex items-center gap-2">
                                <HiOutlineDocumentSearch />
                                <span className='nunito-regular'>For Final Review</span>
                                {Number(data.final_review) > 0 && <Badge className="bg-blue-200 text-slate-800 rounded-xl text-xs">{data.final_review}</Badge>}
                            </div>
                        </Link>
                        {/* <Link className={` ${base} ${url === "/dc/approve-list" ? on : off} `} href='/dc/approve-list'>
                        <div className="flex items-center gap-2">
                            <GrDocumentVerified />
                            <span className='nunito-regular'>Approved</span>
                            {Number(data.approved) > 0 && <Badge className="bg-blue-200 text-slate-800 rounded-xl text-xs">{data.approved}</Badge>}
                        </div>
                    </Link> */}
                    </div>
