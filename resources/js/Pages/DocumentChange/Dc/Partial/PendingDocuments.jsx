...
export default function PendingDocuments() {
    ...
    const fetchDocuments = () => {
        setLoading(true)
        axios.get('/load-pending-documents').then(
            res => {
    ...
    return (
        ...
        <div className="w-full">
            <div className='bg-gray-50 dark:bg-gray-800 text-gray-900 dark:text-gray-50 w-full rounded-lg px-2'>
                        ...
                        currentDocuments.map((item, index) => (
                            <tr key={index} className="bg-gray-50 border-b dark:bg-gray-800 dark:border-gray-700  hover:bg-gray-200 dark:hover:bg-gray-600" >
                                ...
                                <td className="px-6 py-4 text-center">
                                    {process_type.find(pt => Number(pt.value) === Number(item.latest_revision.process_type))?.name || "Unknown Process"}
                                </td>
                                <td className="px-6 py-4 flex justify-center align-middle">
                                    {Number(item.is_final) === 0 && Number(item.latest_revision.progress_status) === 0 ? (
                                        <> <Badge color="teal" icon={HiClock} >
                                            DC Initial Review
                                        </Badge></>
                                    ) : Number(item.is_final) === 0 && Number(item.latest_revision.progress_status) === 1 ? (
                                        <>  <Badge color="lime" icon={HiClock} className='w-fit text-center'>
                                            QMR Review
                                        </Badge></>
                                    ) : Number(item.is_final) === 0 && Number(item.latest_revision.progress_status) === 2 ? (
                                        <>  <Badge color="lime" icon={HiClock} className='w-fit text-center'>
                                            DC Final Review
                                        </Badge></>) : null}
                                </td>
                                <td className="px-6 py-4 text-center space-x-2">
                                    {item.latest_revision.qmr_approved_date === null && (
                                        <PrimaryButton className="px-2 py-1 text-xs bg-yellow-500 text-white" onClick={() => { router.visit('/audit-document/' + item.document_id) }}>Review</PrimaryButton>
                                    )}
                                    <PrimaryButton className="px-2 py-1 text-xs bg-blue-500 text-white ">View</PrimaryButton>
                                    <PrimaryButton className="px-2 py-1 text-xs bg-cyan-500 text-white">Logs</PrimaryButton>
                                </td>
                            </tr>
                        ))
                    ) : (
