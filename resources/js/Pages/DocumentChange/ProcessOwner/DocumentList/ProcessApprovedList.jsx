...
export default function ProcessApprovedList() {
    ...
    const fetchDocuments = () => {
        setLoading(true)
        axios.get('/process/load-approved-documents').then(
            res => {
                        ...
                        currentDocuments.map((item, index) => (
                            <tr key={index} className=" border-b dark:border-gray-700  hover:bg-gray-200 dark:hover:bg-gray-600" >
                                ...
                                <td className=" text-center ">
                                    <PrimaryButton className="m-1 px-2 py-1 text-xs bg-blue-500 text-white" onClick={() => downloadFileFromUrl(item.document_dir)}>{item.file_type === 1 ? 'Word': 'Excel'}</PrimaryButton>
